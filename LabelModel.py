...
class LabelModel(object):
    ...
    _version = "LabelModel version SMI-1.00"

    def __init__(self, label, concept_mapping, max_dimensions=None):
        self._version = LabelModel._version
        self._label = label
    ...
    def deserialise(serialised_file):
        lm = jl.load(serialised_file)
        if LabelModel._version != lm._version:
            raise Exception('LabelModel in %s is version %s which is incompatible with current %s' %
                (serialised_file, lm._version, LabelModel._version))
        return lm
